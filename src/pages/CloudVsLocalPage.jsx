...
import BrevDeployCard from '../components/BrevDeployCard'
import Footer from '../components/Footer'

export default function CloudVsLocalPage() {
  const [ollamaConnected, setOllamaConnected] = useState(false)
  const [brevConfig, setBrevConfig] = useState(() => {
    const key = localStorage.getItem('brevApiKey')
    const endpoint = localStorage.getItem('brevEndpoint')
    const model = localStorage.getItem('brevModelId')
  ...
  const handleBrevConfigured = useCallback((config) => {
    setBrevConfig(config)
  }, [])
  ...
  return (
    <div style={{ maxWidth: 640, margin: '0 auto', padding: '0 24px' }}>
      ...
      <DepthPanel
        ...
        code={`# Cloud inference (Brev / any OpenAI-compatible API)
import openai

client = openai.OpenAI(
    base_url="https://your-deployment.brev.dev/v1",
      ...
      <AnimatePresence>
        {raceComplete && (
          <motion.div
            ...
            style={{ marginTop: 24 }}
          >
            <BrevDeployCard onConfigured={handleBrevConfigured} />
          </motion.div>
        )}
      </AnimatePresence>
